...
public class Simulator {
    ...
    /**
     * Take in these parameters and run the simulate().
     * @param arrivalTimes is a List of all arriveTimes of customers.
     * @param numOfServers is the number of servers.
     */
    public Simulator(List<Double> arrivalTimes, int numOfServers) {
        this.customers = new ArrayList<Customer>();
        this.servers = new ArrayList<Server>();
        for (int i = 0; i < arrivalTimes.size(); i++) {
            double time = arrivalTimes.get(i);
            customers.add(new Customer(i + 1, time));
        }
        for (int j = 0; j < numOfServers; j++) {
            servers.add(new Server(j + 1));
        }
    ...
    /**
     * Call of simulate() method begins the simulation.
     */
